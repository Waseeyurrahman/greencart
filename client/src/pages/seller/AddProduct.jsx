            ...
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img 
                                className="max-w-24 cursor-pointer" src={files[index] ? 
                ...
                <div className="flex flex-col gap-1 max-w-md">
                    ...
                    <textarea onChange={(e)=> setDescription(e.target.value)} value={description}
                     id="product-description" rows={4} className="outline-none
                      md:py-2.5 py-2 px-3 rounded border border-gray-500/40
                ...
                <button type='submit' className="px-8 py-2.5 bg-primary cursor-pointer text-white font-medium rounded">ADD</button>
            </form>
        </div>
  )
}
